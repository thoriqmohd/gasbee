...
export default function Products() {
  ...
  const load = async () => {
    setLoading(true);
    let query = supabase.from("products").select("*").order("created_at", { ascending: false }).limit(300);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h1 className="text-2xl font-bold">Products</h1><p className="text-sm text-muted-foreground">All products from all merchants.</p></div>
      ...
      <Card className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-muted/40"><tr className="text-left text-xs uppercase text-muted-foreground">
            <th className="p-3">Name</th><th className="p-3">Merchant</th><th className="p-3">Size (kg)</th>
            <th className="p-3">Price</th><th className="p-3">Stock</th><th className="p-3">Active</th>
          </tr></thead>
          <tbody>
            {loading ? Array.from({ length: 5 }).map((_, i) => (
              ...
              <tr key={r.id} className="border-t hover:bg-muted/30">
                ...
                <td className="p-3">{r.cylinder_size_kg ?? "—"}</td>
                <td className="p-3">{fmt(r.selling_price)}</td>
