...
export default function UserCompanyVerification() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) return;
    if (!form.ssm_doc_url) { toast.error("Sila upload dokumen SSM."); return; }
    setBusy(true);
    const payload = {
      user_id: user.id,
  ...
  if (existing && existing.status !== "rejected") {
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2"><Building2 className="h-5 w-5 text-primary" /><h1 className="text-lg font-bold">Akaun Syarikat</h1></div>
        <Card className="space-y-2 p-4 text-sm">
          ...
          {existing.status === "pending" && <p className="text-xs">⏳ Permohonan sedang disemak admin.</p>}
          {existing.status === "approved" && <p className="text-xs text-primary">✓ Diluluskan — anda boleh order industrial gas.</p>}
          {existing.review_notes && <p className="text-xs text-muted-foreground">Catatan: {existing.review_notes}</p>}
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2"><Building2 className="h-5 w-5 text-primary" /><h1 className="text-lg font-bold">Daftar Akaun Syarikat</h1></div>
      <p className="text-sm text-muted-foreground">Untuk membeli industrial gas, sila daftar syarikat dan upload dokumen SSM untuk pengesahan.</p>

      {existing?.status === "rejected" && (
        <Card className="border-destructive bg-destructive/10 p-3 text-sm">
          Permohonan sebelum ini ditolak. {existing.review_notes && <span>Catatan: {existing.review_notes}</span>}
        </Card>
      )}

      <Card className="p-4">
        <form onSubmit={submit} className="space-y-3">
          <div><Label>Nama syarikat *</Label><Input required maxLength={200} value={form.company_name} onChange={(e) => setForm({ ...form, company_name: e.target.value })} /></div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <div><Label>Telefon</Label><Input maxLength={30} value={form.contact_phone} onChange={(e) => setForm({ ...form, contact_phone: e.target.value })} /></div>
          </div>
          <div><Label>Alamat perniagaan</Label><Textarea maxLength={500} value={form.business_address} onChange={(e) => setForm({ ...form, business_address: e.target.value })} /></div>
          <div>
            <Label>Dokumen SSM *</Label>
          ...
          <div>
            <Label>Dokumen tambahan (optional)</Label>
